...
impl LlmTool for GetTextTool {
    ...
    async fn deep_seek_call(&self, tool_call: &ToolCall) -> String {
        let args: GetTextArgs = match parse_tool_args(tool_call, self.base_tool.name.as_str()) {
            Ok(args) => args,
            Err(err) => return err,
        };
...
mod tests {
    ...
    #[async_trait::async_trait]
    impl BrowserDriverFactory for GetTextFactory {
        async fn create(&self) -> anyhow::Result<Box<dyn BrowserDriver>> {
            Ok(Box::new(GetTextDriver {
    ...
    #[async_trait::async_trait]
    impl BrowserDriver for GetTextDriver {
    ...
    fn get_text_call(arguments: &str) -> ToolCall {
        ToolCall {
            id: "call_get_text".to_string(),
    ...
    #[tokio::test]
    async fn get_text_returns_invalid_arguments_for_bad_json() {
        let tool = GetTextTool::new();
    ...
    #[tokio::test]
    async fn get_text_rejects_unknown_arguments() {
        let tool = GetTextTool::new();
        let result = tool
    ...
    async fn get_text_calls_driver_with_element_id() {
        ...
        let raw = run_with_browser_scope(factory.clone(), async {
            Ok::<_, anyhow::Error>(
                tool.deep_seek_call(&get_text_call(r#"{"element_id":"node-1"}"#))
                    .await,
            )
        })
        .await
    ...
    #[tokio::test]
    async fn get_text_wraps_driver_error_as_browser_error_json() {
        let tool = GetTextTool::new();
        let raw = run_with_browser_scope(Arc::new(ErrorFactory), async {
            Ok::<_, anyhow::Error>(
                tool.deep_seek_call(&get_text_call(r#"{"element_id":"missing"}"#))
                    .await,
            )
        })
        .await
