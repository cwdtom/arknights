...
以下内容主要用来说明非开源版本会继续扩展的能力，也帮助说明整体产品
方向；当前开源仓库仍然聚焦上面已经列出的这些能力。

- `非开源版本实现` 接入例如 GPT、Claude 等 LLM
- `非开源版本实现` IM 接入 WeChat
- `非开源版本实现` 通过设置 Cookie，让 agent 访问需要登录的网站，例如知乎、B 站
- `非开源版本实现` 支持创建、下载 Skill 并执行
- `非开源版本实现` 支持添加 MCP 工具
